...
var ROOT        = nconf.get('root') || process.cwd();
...
Loader.prototype.resolve = function(path, callback) {
    ...
    async.waterfall([
        ...
        function (originalPath, next) {
            ...
            fs.exists(ROOT + '/' + originalPath.toString().replace(/\./g,'/') + '.js', function(exists) {
                if (exists) {
                    self.create(originalPath, ROOT + '/' + originalPath.toString().replace(/\./g,'/') + '.js', function(obj) {
                        next(obj);
                    });
                } else {
                    next(null, originalPath);
        ...
        function (originalPath, next) {
            ...
            fs.exists(ROOT + '/' + originalPath.getPackage().replace(/\./g, '/'), function(exists) {
                if (exists) {
                    next(null, originalPath);
                } else {
        ...
        function (originalPath, next) {
            ...
            var defaultComponentPath = ROOT + '/' + originalPath.clone()
        ...
        function (originalPath, next) {
            ...
            var defaultCompontentPathDefault = ROOT + '/' + originalPath.clone()
                .setComponent('default')
                .setName('default')
                .toString()
                .replace(/\./g, '/') + '.js'
